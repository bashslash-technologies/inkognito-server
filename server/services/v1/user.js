...
async function loginUser({ username, password, role }) {
  try {
    ...
    if (__user.verification.code) {
      ...
      let __expiry = new Date().setTime(
        new Date().getTime() + config.auth.verification_expiry
      );
    ...
    if (__user.isLocked)
      throw new Error(
        "sorry your account has been locked, kindly retry after " +
          moment().calendar(__user.locker.expiry)
      );
    console.log("got here");
    let isValid = await __user.comparePasswords(password);
    if (!isValid) {
      const __expiry = new Date(
        new Date().getTime() + config.auth.reset_expiry
      );
      await __user.updateOne({
        $inc: {
          "locker.tries": 1,
...
async function verifyReset({ username, code }) {
  try {
    ...
    if (new Date() > new Date(__user.reset.expiry))
      throw new Error("reset code expired");
    console.log(__user.reset.code, code);
