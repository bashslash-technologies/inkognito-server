...
const { handleSuccess, resolveCourier } = require("../../middlewares");
...
function init(io) {
  ...
  router.get("/open", resolveCourier, async function (req, res, next) {
    try {
  ...
  router.get("/courier", async function (req, res, next) {
    try {
      let result = await tripService.retrieveCourierTrips(
        req.user_id,
        req.query
      );
      handleSuccess(res, result);
